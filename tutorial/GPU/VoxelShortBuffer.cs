...
namespace tutorial.GPU
{
    ...
    public struct dVoxelShortBuffer
    {
        ...
        public void SetVoxel(Ray ray, float depth, Voxel color, bool isLeft, int depthMult, int maxFilledVoxels)
        {
            int filledVoxelCount = 0;

            if (aabb.hit(ray, 0.001f, float.MaxValue))
            {
                Vec3 pos = ray.a;

                Vec3i iPos = new Vec3i(XMath.Floor(pos.x), XMath.Floor(pos.y), XMath.Floor(pos.z));
                float stepValue = 0.99f;
                Vec3 step = new Vec3(ray.b.x > 0 ? stepValue : -stepValue, ray.b.y > 0 ? stepValue : -stepValue, ray.b.z > 0 ? stepValue : -stepValue);
                ...
                bool hit = false;
                Vec3 firstHit = new Vec3();

                while (i < max)
                {
                    Vec3 offsetPos = pos - position;
                    offsetPos.z = length - offsetPos.z;
                    if (offsetPos.isWithin(new Vec3(width, height, length)))
                    {                       
                        if(!hit)
                        {
                            firstHit = offsetPos;
                        ...
                        {
                            if(Vec3.dist(firstHit, offsetPos) >= depth * depthMult)
                            {
                                ...
                                if (isLeft)
                                {
                                    tile.SetAfterLeftCameraData(true);
                                }
                                else
                                {
                                    tile.SetAfterRightCameraData(true);
                                }

                                writeFrameBuffer((int)offsetPos.x, (int)offsetPos.y, (int)offsetPos.z, tile);

                                if (filledVoxelCount > maxFilledVoxels)
                                {
                                    return;
                                }
                                else
